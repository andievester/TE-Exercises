...
-- (2 rows)
SELECT film.title, category.name AS category, actor.first_name, actor.last_name
FROM film
...
SELECT store.store_id, address.address, SUM(payment.amount),  COUNT(payment.amount), AVG(payment.amount)
FROM store
JOIN address ON address.address_id = store.address_id
...
SELECT film.title, count(rental.*) AS top_rented_films
FROM film
JOIN inventory ON inventory_id = film.film_id
...
SELECT category.name, count(rental.*) AS top_rented_categories
FROM film
...
ORDER BY top_rented_categories DESC
LIMIT 5;

-- 18. The top five Action film titles by number of rentals 
-- (#1 should have 30 rentals and #5 should have 28 rentals)
SELECT film.title, count(rental.*) AS top_rented_action_films
FROM film
JOIN inventory ON inventory.film_id = film.film_id
JOIN rental ON rental.inventory_id = inventory.inventory_id
JOIN film_category ON film_category.film_id = film.film_id
...
GROUP BY film.title
ORDER BY top_rented_action_films DESC
LIMIT 5;

-- 19. The top 10 actors ranked by number of rentals of films starring that actor 
-- (#1 should be “GINA DEGENERES” with 753 rentals and #10 should be “SEAN GUINESS” with 599 rentals)
SELECT actor.first_name, actor.last_name, COUNT(rental.*) AS top_rented_actors
FROM actor
...
ORDER BY top_rented_actors DESC
LIMIT 10;

-- 20. The top 5 “Comedy” actors ranked by number of rentals of films in the “Comedy” category starring that actor 
-- (#1 should have 87 rentals and #5 should have 72 rentals)
SELECT actor.first_name, actor.last_name, COUNT(rental.*) AS top_rented_comedy_actors
FROM actor
JOIN film_actor ON film_actor.actor_id = actor.actor_id
JOIN film ON film.film_id = film_actor.film_id
JOIN inventory ON inventory.film_id = film.film_id
...
WHERE category.name = 'Comedy'
GROUP BY actor.actor_id, actor.last_name, actor.first_name
ORDER BY top_rented_comedy_actors DESC
LIMIT 5;