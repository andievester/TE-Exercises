...
SELECT  ROUND(((gnp * 1000000) / population)::numeric, 2) as "Per Capita GNP"
FROM country 
WHERE population != 0
...
SELECT ROUND(avg(population):: numeric, 2), countrycode
FROM city
...
SELECT ROUND(avg(lifeexpectancy):: numeric, 4), MIN(lifeexpectancy), MAX(lifeexpectancy), continent
FROM country
GROUP BY continent
